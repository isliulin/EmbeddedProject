...
//const uint32_t gc_ulBaudrate = 921600;
...
//WIFI AP模式,模块对外的无线参数
char *g_pcAPSSID = "DDF20-1";                //SSID名称
//char *g_pcAPSSID = "aaaDDF20A";                //SSID名称
...
uint8_t WaitATCmdResponse(char *pcAckStr, uint16_t usTimeOut)
{
    ...
    {
        if(strstr((char *)g_ucaUsart1RxBuf, (char *)pcAckStr))
        {
            ucSta = TRUE; 
            break;
        }
