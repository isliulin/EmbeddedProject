...
//    pucMX25Lxx_BUF = pucBuf;
    ...
    for ( ;; )
    {
        MX25Lxx_Read( pucBuf, ulSectionLocation * 4096, 4096 ); //读出整个扇区的内容
        ...
        if ( i < usSectionRemain ) //需要擦除
        {
			...
			/*MX25Lxx_Read( pucBuf, ulSectionLocation * 4096, 4096 ); //读出(测试用，完后删除)
			for(i = 0; i < 4096; i++)
			{
				pucBuf[i] = 0xFF;
			}*/
            ...
            MX25Lxx_WriteNoCheck( pucBuf, ulSectionLocation * 4096, 4096 ); //写入整个扇区
        }
        else
        {
